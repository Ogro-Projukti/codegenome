...
class LiveSession:
    ...
    def open_browser(self) -> str:
        ...
        karyotype_url = f"http://localhost:{http_port}/karyotype.html{live_query}"
        if self._config.lan:
            lan_ip = get_lan_ip()
            lan_url = f"http://{lan_ip}:{http_port}/graph.html{live_query}"
            self._emit(f"HTTP server listening on http://0.0.0.0:{http_port}")
            self._emit(f"  Graph (local):    {local_url}")
            self._emit(f"  Graph (LAN):      {lan_url}")
            self._emit(f"  Karyotype view:   {karyotype_url}")
        else:
            self._emit(
                f"HTTP Server started. Opening live graph at {local_url}..."
            )
            self._emit(
                f"  Genome DNA karyotype available via Explore Karyotype or {karyotype_url}"
            )
