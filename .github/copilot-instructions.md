...
Verify the server: `http://127.0.0.1:7331/health` should report `"memory_bounded": true` and a non-empty `snapshot_id` after `codegenome analyze`.
...
- Verify your MCP usage by monitoring tool call success. If native tools are missing, try the configured local HTTP MCP endpoint when possible. If both native and HTTP MCP access fail, politely ask the user to configure their editor's MCP settings to run `codegenome mcp-start --path <workspace> --memory-bounded` (stdio) or start HTTP with `--transport http --port 7331 --memory-bounded`.
